...
pub enum OclDeviceType { CPU, GPU, ACCEL, ALL }
...
pub fn get_avali_ocl_devices(ty: OclDeviceType)->Result<Vec<(platform::Platform, Vec<device::Device>)>>{
    ...
    for plat in platforms{
        devices.push((plat, plat.get_devices(sys_ty)?.iter().copied().map(device::Device::new).collect()));
    }
    Ok(devices)
}
...
impl OclRunConfig{
    pub fn new(ty:OclDeviceType)->Result<OclRunConfig>{
        Ok(OclRunConfig{devices:get_avali_ocl_devices(ty)?})
    }
    ...
    pub fn get_device_strs(&self)->Vec<String>{
        if self.is_empty(){
            ...
            vec![format!("Rust CPU implementation using {} threads", n_threads)]
        }
        else{
            self.devices.iter().map(|(_,devs)|devs.iter()).flatten().map(|d|d.name().unwrap()).collect()
        }
    }
